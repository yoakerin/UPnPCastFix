...
### **步骤4：删除typealias定义**
**目标**：移除typealias，直接使用lambda表达式

**操作**：
1. 删除：`typealias DLNAResult = (success: Boolean) -> Unit`
2. 删除：`typealias DLNADeviceList = (devices: List<DLNACast.Device>) -> Unit`
3. 删除：`typealias DLNADeviceSelector = (devices: List<DLNACast.Device>) -> DLNACast.Device?`
4. 更新所有方法签名使用直接的lambda类型
...
### **步骤5：简化Device数据类**
**目标**：将Device从7个属性精简为4个核心属性

**操作**：
1. 修改`DLNACast.Device`：
   ```kotlin
   data class Device(
       val id: String,
       val name: String,
